...
public class QuickSort<E extends Comparable<E>> implements Sorter<E> {
    ...
    private int partitionDec(int lo, int hi) {
        ...
        while (j >= i) {
            ...
            while (i <= j && elements.get(i).compareTo(pivot) >= 0) i++; // look for smaller than pivot
            while (i <= j && elements.get(j).compareTo(pivot) < 0) j--; // look for larger than pivot

            if (j > i) swap(i, j); // don't want to swap if the indices have crossed
        }

        // Don't want to swap in the case that the lo-index element is already in the correct spot (largest), so hide behind conditional
        if (pivot.compareTo(elements.get(j)) < 0) {
            swap(j, lo);
            steps.append("  :: ");
            steps.append(elements);
